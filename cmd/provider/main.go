...
import (
	...
	apisCluster "github.com/unbounded-tech/provider-authentik/apis/cluster"
	apisNamespaced "github.com/unbounded-tech/provider-authentik/apis/namespaced"
	"github.com/unbounded-tech/provider-authentik/config"
	"github.com/unbounded-tech/provider-authentik/internal/clients"
	controllerCluster "github.com/unbounded-tech/provider-authentik/internal/controller/cluster"
	controllerNamespaced "github.com/unbounded-tech/provider-authentik/internal/controller/namespaced"
	"github.com/unbounded-tech/provider-authentik/internal/features"
	"github.com/unbounded-tech/provider-authentik/internal/version"
)
...
func main() {
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Authentik APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Authentik APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-authentik:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Authentik controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Authentik controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Authentik controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Authentik controllers")
	}
